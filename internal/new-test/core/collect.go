...
func Collect(flow *questions.Question, result *Result) []Activation {
	var acts []Activation
	collectStep(flow, result, &acts)
	return acts
}

func collectStep(question *questions.Question, result *Result, acts *[]Activation) {
	if question == nil {
		return
	}
...
func evaluateCondition(actual string, comp questions.Comparison, expected string) bool {
	switch comp {
	case questions.ComparisonEqual:
		return actual == expected
	case questions.ComparisonNotEqual:
		return actual != expected
	default:
		return false
	}
